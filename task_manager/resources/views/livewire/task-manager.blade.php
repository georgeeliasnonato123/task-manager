            ...
            <!-- Form para adicionar tarefa -->
                ...
                <form wire:submit.prevent="storeCategory" id="newCategoryField" style="display: none;" action="{{ route('store.category') }}" method="POST">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <button type="button" class="ml-2 bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" onclick="document.getElementById('newCategoryField').style.display='none'">Fechar</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="overflow">
                <table class="table-auto w-full">
                    ...
                    <tbody>
                        @if($tasks->isEmpty())
                            <tr>
                                <td colspan="4" class="border px-4 py-2 text-center">Nenhuma tarefa encontrada</td>
                            </tr>
                        @else
                            @foreach($tasks as $task)
                                <tr>
                                    ...
                                    <td class="border px-4 py-2">
                                        ...
                                        <div id="modal{{$task->id}}" class="fixed z-10 inset-0 overflow-y-auto hidden">
                                            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                                                <div class="fixed inset-0 transition-opacity">
                                                    <div class="absolute inset-0 bg-gray-500 opacity-75"></div>
                                                </div>
                                                        ...
                                                        <div class="sm:flex sm:items-start">
                                                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                                                ...
                                                                <form action="{{ route('tasks.destroy', $task->id) }}" method="POST" class="mt-2">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-500 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:w-auto sm:text-sm">
                                                                        Confirmar Exclusão
                                                                    </button>
                                                                </form>
                                            ...
                                            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                                                ...
                                                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                                                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                                        <div class="sm:flex sm:items-start">
                                                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                                                <button type="button" class="absolute top-2 right-2 text-gray-500 hover:text-gray-700" onclick="closeEditModal()">
                                                                    &#10006;
                                                                </button>
                                                                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-headline">
                                                                    Editando Tarefa: <span id="taskId"></span>
                                                                </h3>
                                                                <form id="editTaskForm" action="{{ route('update.task', ['task' => '__TASK_ID__']) }}" method="POST" class="mt-2">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    <input type="hidden" name="task_id" id="task_id">
                                                                    <div class="mb-4">
                                                                        <label for="editTitle" class="block text-sm font-medium text-gray-700">Título da Tarefa:</label>
                                                                        <input type="text" name="editTitle" id="editTitle" class="mt-1 block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Título da Tarefa" value="{{ $task->title }}">
                                                                    ...
                                                                    <div class="mb-4">
                                                                        ...
                                                                        <textarea name="editDescription" id="editDescription" class="mt-1 block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Descrição">{{ $task->description }}</textarea>
                                                                    </div>
                                                                    <div class="mb-4">
                                                                        <label for="editCategory" class="block text-sm font-medium text-gray-700">Categoria:</label>
                                                                        <select name="editCategory" id="editCategory" class="mt-1 block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                                            <option disabled selected>Selecione uma categoria</option>
                                                                            @foreach($categories as $category)
